...
window.addEventListener('popstate', (e) => {
    ...
    else if (document.getElementById('canvas-jump-bar') && !document.getElementById('canvas-jump-bar').classList.contains('hidden')) { window.hideJumpBar(true); }
...
window.toggleJumpBar = function() {
    ...
    if (bar.classList.contains('hidden')) {
        ...
        pushAppHistory('jump-bar');
    } else {
        if (window.location.hash === '#jump-bar') { history.back(); }
        else { window.hideJumpBar(true); }
    }
};

// Dipanggil dari popstate (fromHistory=true) ATAU langsung (fromHistory=false, mis. setelah eksekusi jump)
window.hideJumpBar = function(fromHistory = false) {
    const bar = document.getElementById('canvas-jump-bar');
    if (!bar || bar.classList.contains('hidden')) return;

    const input = document.getElementById('canvas-jump-input');
    // Jika dipicu dari tombol back & keyboard masih terbuka (input fokus):
    // tutup keyboard dulu, jangan tutup bar-nya. Back berikutnya baru menutup bar.
    if (fromHistory && input && document.activeElement === input) {
        input.blur();
        // Pertahankan state #jump-bar di history agar back selanjutnya bisa menutup bar
        pushAppHistory('jump-bar');
        return;
    }

    bar.classList.add('hidden');
    if (!fromHistory && window.location.hash === '#jump-bar') history.back();
};

// Tutup jump bar saat tap di luar (layar kosong), kecuali tombol trigger "#"
// Cukup trigger history.back() — popstate akan menutup bar (juga handle keyboard-first di atas)
document.addEventListener('mousedown', (e) => {
    const bar = document.getElementById('canvas-jump-bar');
    if (!bar || bar.classList.contains('hidden')) return;
    const target = e.target;
    if (bar.contains(target)) return;
    if (target && target.closest('[onclick*="toggleJumpBar"]')) return;
    if (window.location.hash === '#jump-bar') history.back();
    else window.hideJumpBar(true);
});
...
window.toggleFullscreenReading = function(isFromHistory = false) {
    ...
    if (bottomBar.classList.contains('hidden')) {
        ...
        if (canvasCtrl) canvasCtrl.classList.add('hidden');
        window.hideJumpBar(true); // sembunyikan jump bar saat immersive (tanpa history.back tambahan)
        updateBottomNavUI(null);
